...
if (isset($_GET['aid'])) {
    ...
    $_SESSION['error'] = "⚠️ **User Not Found:** No user ID provided.";
    header("Location: index.php");
    exit();
}

//check if user id is valid number or not or empty
if (empty($user_id) || !is_numeric($user_id)) {
    $_SESSION['error'] = "⚠️ **Invalid User ID:** The user ID provided is not valid.";
    //if there is any error redirect to current page
    header("Location: index.php");
    exit();
} else {
    $user_id = (int) $user_id; //cast to integer for safety

    //check valid user id or does it exist in database
    $user_check_sql = "SELECT * from user where user_id = {$user_id}";
    $user_check_result = mysqli_query($conn, $user_check_sql);

    if (!$user_check_result || mysqli_num_rows($user_check_result) == 0) {
        $_SESSION['error'] = "⚠️ **User Not Found:** The requested user does not exist.";
        //if there is any error redirect to current page
        header("Location: index.php");
        exit();
    } else {
...
if ($page < 1) {
    ...
    header("Location: $_SERVER[PHP_SELF]?aid=" . $user_id . "&page=" . 1);
...
if (!$conn) {
    ...
    if ($countResult && mysqli_num_rows($countResult) > 0) {
        ...
        if ($page > $total_pages && $total_pages > 0) {
            ...
            $_SESSION['error'] = "⚠️ **Page Not Found:** You tried to access page " . ($page) . ", but it does not exist.";
            header("Location: " . $_SERVER['PHP_SELF'] . "?aid=" . $user_id . "&page=" . $total_pages);
            exit(); // CRITICAL: Stop script so redirect happens immediately
        }
    }
}
                ...
                <div class="post-container">
                    ...
                    if ($conn):
                        ...
                        if (!$result):
                            ...
                            <h2 class="page-heading"><?php echo $username; ?></h2>
                    ...
                    <ul class='pagination'>
                        ...
                        if ($total_pages > 1):
                            //Previous Page
                            if ($page > 1):
                                echo '<li class="prev"><a href=" ' . $_SERVER['PHP_SELF'] . '?aid=' . $user_id . '&page=' . ($page - 1) . ' " >Prev</a></li>';
                            endif;
                            ...
                            // Start Ellipsis
                            if ($start > 1):
                                echo '<li><a href=" ' . $_SERVER['PHP_SELF'] . '?aid=' . $user_id . '&page=1">1</a></li>';
                                if ($start > 2)
                            ...
                            for ($i = $start; $i <= $end; $i++):
                                ...
                                echo '<li class=" ' . $active . ' "><a href=" ' . $_SERVER['PHP_SELF'] . '?aid=' . $user_id . '&page=' . $i . ' " >' . $i . '</a></li>';
                            endfor;
                            ...
                            if ($end < $total_pages):
                                if ($end < $total_pages - 1)
                                    echo '<li><span>...</span></li>';
                                echo '<li><a href=" ' . $_SERVER['PHP_SELF'] . '?aid=' . $user_id . '&page=' . $total_pages . '">' . $total_pages . '</a></li>';
                            endif;

                            //next page
                            if (1 < $total_pages && $page < $total_pages):
                                echo '<li class="next"><a href=" ' . $_SERVER['PHP_SELF'] . '?aid=' . $user_id . '&page=' . ($page + 1) . ' " >Next</a></li>';
                            endif;
                        else:
