...
try {
    ...
    $countResult = db()->select("user", "COUNT(user_id) AS total_users");
    $total_users = $countResult->fetch_assoc()['total_users'];
    ...
    $result = db()->select(
        "user",
        "*",
        "1=1 ORDER BY user_id ASC LIMIT ? OFFSET ?",
        [$limit, $offset]
    );

} catch (Exception $e) {
    // --- KEY CHANGE: Capture Exception in Session ---
    // We store the error message from the Database class into the session
    // OPTION A: Show a generic UI message for ALL database errors
    $_SESSION['error'] = "Something went wrong on our end. Our team has been notified.";

    /* OPTION B: Use the technical error ONLY if you are in local development, 
       otherwise show the generic one.
    */
    // $_SESSION['error'] = "Data Error: " . $e->getMessage();
