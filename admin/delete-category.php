...
if (isset($_GET['id'])) {
   ...
   //--- Check connection error ---
    ...
    if (!$check_result || mysqli_num_rows($check_result) == 0) {
        ...
        $_SESSION['error'] = "🔍 **Record Not Found:** We could not find a student with ID **" . htmlspecialchars($category_id) . "** to delete. Please verify the ID.";
        header("Location: category.php");
        mysqli_close($conn);
        exit();
    } else {
    ...
    // --- Delete Records---
    //Delete SQL
    $delSql = "DELETE FROM category where category_id = {$category_id}";
    ...
    if ($del_result) {
        ...
        if ($deleted_count > 0) {
            $_SESSION['success'] = "✅ **Success!** Record for ID **" . htmlspecialchars($category_id) . "** has been successfully deleted.";
        } else {
            // This handles a successful query that deleted 0 rows 
            // (e.g., if ID was deleted between check and delete)
            $_SESSION['error'] = "❌ **Deletion Failed:** The record for ID **" . htmlspecialchars($category_id) . "** could not be deleted at this time, possibly due to a recent change.";
        }
    } else {
