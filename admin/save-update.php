...
if (isset($_POST['submit'])) {
    ...
    $errors = array();
    //check if the updated file is new or old
    ...
    //input Validation
    if (empty($post_title) || empty($postDesc) || empty($category) || empty($author)) {
    ...
    $sql = "UPDATE post 
            SET title = '$post_title', description = '$postDesc', category = '$category', 
            post_date = '$date', author = '$author', post_img = '$image_name' 
            WHERE post_id = {$_POST['post_id']};";

    //if current is changed then update the post count of the categories
    ...
    if (mysqli_multi_query($conn, $sql)) {
        ...
        header("Location: update-post.php?id=" . $_POST['post_id']);
        exit();
    }
    mysqli_close($conn);
